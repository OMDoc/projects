...
GivenFormulasIn[_]:={};
CurrentPath={};
CurrentSection:=CurrentPath\[LeftDoubleBracket]1\[RightDoubleBracket];
    ...
    (
        ...
        AddUpperBound[
          form\[LeftDoubleBracket]1\[RightDoubleBracket]-
            form\[LeftDoubleBracket]2\[RightDoubleBracket],0,section]];
      	If[Head[form]==Less,
        AddUpperBound[
          form\[LeftDoubleBracket]1\[RightDoubleBracket]-
            form\[LeftDoubleBracket]2\[RightDoubleBracket],Strict[0],
          section]];
      	If[Head[form]==Equal,AddIdentity[form,section]];
      	AddSimplifyRules[form,section];
      );



GivenIdentities:=Union@@GivenIdentitiesAt/@CurrentPath;



RulesFromGiven:=Union@@RulesGivenIn/@CurrentPath;
GivenFormulas[]:=Union@@GivenFormulasIn/@CurrentPath;



GivenUpper[n_]:=Union@@(GivenUpperAt[#1][n]&)/@CurrentPath;
GivenLower[n_]:=Union@@(GivenLowerAt[#1][n]&)/@CurrentPath;
...
OpenDefinition[seq_]:=
    Block[{s0},
      	s0=Skolemize[seq/.\[InvisibleSpace]UserFunctions,NegPos,{}];
...
ProveByInduction[{n_,n0_,nbases_},f_]:=
    Module[{k},(
        ...
        For[k=0,k<nbases,k++,
          ...
          If[   FalseQ[ 
              Verify[ seq[ True,
                  Skolemize[ f/.\[InvisibleSpace](n\[Rule]n0+k),
                    NegPos,{}]]]],
...
Verify[s_]:=
    Block[{sequent},
      	sequent=StrongSimplify[s];
      	PrintMessage[SimpMsgList];
      	If[DepthCount>0,Return[TryProving[sequent]]];
      	orelse[TryProving[sequent],
...
RewriteAndProve[s_]:=
    ...
    orelse[ TryProveIfNotSame[sequent,sequent=SubstEquation[sequent]],
...
TryProving[s_]:=
    Block[{temp,temph},
      	PrintSequent[s];
      	SucceedWith[ If[TrueQ[s],TryOtherBranches[True],
...
Block[{u}, u /; !FalseQ[u = Backchain[seq[h0, or[c0, c1]], Lemmas[Head[c]], c]]];
Imply[seq[h0_, c_]] := 
Block[{u}, u /; !FalseQ[u = Backchain[seq[h0, False], Lemmas[Head[c]], c]]];
...
GetBase[x_, f_] := EvaluateAssuming[ not[f], 
	  If[ !TrueQ[ WeakSimplify[integer[x]]] || 
	  		!useful[x, f, StrongSimplify[f/.x->x+1]], -Infinity, SelectBase[Lower[x]]]];
...
OperatorRules = { Or -> or, And -> and, Not -> not };
...
IsVarConsSymbol[a_] := Head[a]===Var || Head[a]===Const || Head[a]===Symbol;
